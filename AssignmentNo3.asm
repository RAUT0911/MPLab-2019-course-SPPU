section .data
    ...
    nwline db 10
    array dq 0fa10001h,0b200002h,0fff0003h,0d400004h,0fffffffh  
    arrcnt dd 05h
...
_start:
	...
	mov rsi,array ; si pointing to	
	mov rcx,[arrcnt] ; cx contains the no of elements in the

	mov rax,[rsi] ; starting addr copied to ax
	dec rcx ; cx decremented by 1
lup1: add rsi,08 ;Point to next element
	cmp rax,[rsi] ;compare value present at address ax with next

 ; CMP subtracts the second operand from the

	ja lskip1 ; jump if unsigned above, CF=0, skip the
...
dispskip1:
	...
	loop dispup1 ;decrement the count of digits to
